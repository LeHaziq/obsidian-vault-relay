...
export interface Relay {
  ...
  /** Binds the socket and resolves with the port that was bound. */
...
export function createRelay(config: Config, options: RelayOptions = {}): Relay {
  ...
  // Without a listener a server error becomes an uncaught exception, so a
  // socket failure after startup would take the process down.
  ...
  return {
    store,
    listen: (port, host) => listen(server, port, host),
